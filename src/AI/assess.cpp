...
void BoxBoard::defineAllChains(bool ChainPlus) // 定义所有的链
{
    ...
    // 短链检测: 未被注册为环的孤立链格 (1-chain / 2-chain)
    // BoxesZero 扩展终局定理: 将经典理论未覆盖的短路链纳入可控值计算
    for (int i = 1; i <= BOXLEN; i++)
    {
        for (int j = 1; j <= BOXLEN; j++)
        {
            if (Boxes[i][j].Type == CHAINBOX && Boxes[i][j].BelongingChainNum == EMPTY)
            {
                ...
                Boxes[i][j].BelongingChainNum = Re;

                // 尝试沿一个方向扩展 (2-chain 探测)
                for (auto n : Dir)
                {
                    int bx = i + n[0];
                    int by = j + n[1];
                    int lx = (i * 2) - 1 + n[0];
                    int ly = (j * 2) - 1 + n[1];
                    if ((map[lx][ly] == HENG || map[lx][ly] == SHU) &&
                        getBoxType(bx, by) == CHAINBOX &&
                        Boxes[bx][by].BelongingChainNum == EMPTY)
                    {
                        Boxes[bx][by].BelongingChainNum = Re;
                        Chains[Re].ChainBoxNum = 2;
                        break;
                    }
                }
            }
...
LOC BoxBoard::getEarlyRationalBoxNum() // 用于UCT预处理，获得余下局面双方以理性状态可以获得的格子数量
{
    ...
    for (int i = 1; i <= 25; i++)
    {
        if (Chains[i].Type != NotDefine)
        {
            if (Chains[i].Type == LongChain || Chains[i].Type == ShortChain)
            {
                LCNum++;
    ...
    int Sacrifice = 0;
    if (LCNum == 0 && CNum != 0) // 此状况下不存在链，则都是环
    {
